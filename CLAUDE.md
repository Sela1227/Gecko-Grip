...
- **版本：** V0.4.0
- **狀態：** 可運作
- **一句話定位：** 個人英文單字間隔重複練習工具，英文顯示 → 中文遮蔽 → Space 揭示，Leitner 五盒演算法，雙詞庫（通用萬字 + 醫學專業）
...
- [ ] Space 揭示中文 + 評分按鈕出現
- [ ] T 開拼字輸入，Enter 確認，醫學詞彙接受縮寫（如 SBRT）
- [ ] 1/2 評分後進下一張
- [ ] 設定儲存後重整仍保留
...
1. **揭示前真正主動回想：進入拼字輸入前先遮蔽英文** — 目前拼字測試在揭示中文後、英文仍可見，等於照抄不是回想；下版應允許「先輸入拼字 → 再 Space 揭示」的順序，才構成真正的主動記憶測試
2. 醫學詞彙支援分類篩選（只練 RAD / 只練 RES 等）
3. 個別詞可標記「暫時略過」（太難或太生僻）
4. 回合結束後顯示答錯詞列表供複習
...
V0.4.0：更名為 Gecko Grip（壁虎抓力 = 單字黏住不放），UI 加中文副標，localStorage key 凍結保住既有進度（P6），補回靜默掉的兩個醫學詞並為詞庫加上筆數斷言（P7）。下版第一優先是「揭示前主動回想：先輸入拼字再揭示，測真正的主動記憶」。
