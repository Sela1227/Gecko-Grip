...
6. **把坑 P6–P14 回流到 Kit `cross-project-pitfalls.md`** — 已累積 6 條，其中 P10 是 Kit 既有坑 #3 的第二次現形（可補強原條目）、P7 與 P10 同屬「同一件事寫兩遍」家族，值得合併成一條通則
7. 醫學詞庫外移成獨立 `medical.js`（classic script 設 window 全域，維持 file:// 相容），降低人工搬移掉詞風險

---

## 八、一句話總結

V0.7.0：新字改依詞性配比選取（預設動 45／名 35／形 20），主要詞性改用 Brown 語料庫實測而非字典排版順序（修正 24% 誤判，坑 P12），並排除佔全表 25.2% 的變化形（坑 P13）。下版第一優先是「核心高頻動詞改出搭配卡」—— 因為 `get` → 「得到」這張卡教不了 `get better`／`get married`，而那才是高頻動詞的真正難處。
